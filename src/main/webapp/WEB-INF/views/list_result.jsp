	...
	<body>
			...
			<div class="content_wrap">
				<div id="content">
					...
					<div class="popular_movie my_list">
						<h2>${categoryPageVO.getSearch()}&nbsp;<em>About ${paging.getTotalCount()} results</em></h2>
						<!-- <div class="search">
							...
							<div class="list_top_right">
								...
								<div class="by">
									...
									<div class="drop_menu">
										...
										<ul>
											<li><a onClick="sortPage('reg_date')">Upload date</a></li> <!-- 수정일 -->   
											<li><a onClick="sortPage('view_count')">View count </a></li> <!-- 추후변경 -->
										</ul>
									</div>
								</div>
							</div>
				...
				<c:forEach var="content" items="${list}" varStatus="status">
					<c:choose>
						<c:when test="${status.count % 5 == 1}">
							<li class="mr24">
								<a id="goDetail" data-contentId="${content.getContent_id()}" data-playlistName="${content.getTag().getCategory()}">
									<span>
										<img width="196px" height="110px" src="${content.getThumb_url()}" alt="" />
										<span class="video-time">${content.getDuration()}</span>
								...
								<c:choose>
								<c:when test="${fn:length(content.getTitle()) > 37}">
								<h3><a>${fn:substring(content.getTitle(), 0, 37)}...</a></h3>
								</c:when>
								<c:otherwise>
								<h3><a><c:out value="${content.getTitle()}"/></a></h3>
								</c:otherwise>
								</c:choose>
								<span><c:out value="${content.getView_count()}"/> views</span>
								<span class="f_left"><c:out value="${content.getReg_date()}"/></span>
						...
						<c:when test="${status.count % 5 == 2 || status.count % 5 == 3}">
							<li class="mr24">
								...
								<c:otherwise>
								<h3><a>${content.getTitle()}</a></h3>
								</c:otherwise>
								</c:choose>
								<span><c:out value="${content.getView_count()}"/> views</span>
								<span class="f_left">${content.getReg_date()}</span>
								<span class="f_right mr5">
									<a class="download_btn" data-contentId="${content.content_id }">
										<img src="./resources/images/common/dow_icon.png" alt="download" />
									</a></span>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				// thumb nail click 하면 detail page로 이동.
			...
			function sortPage(sort) {
				...
				var hiddenSec	= null;
				var hiddenSrt	= null; 
				searchValue = '<c:out value="${categoryPageVO.getSearch()}"/>';
				mamCook = $.cookies.get('mamsCookie');
				hiddenHis = $('<input>').prop({'type':'hidden','name':'historyList','value':JSON.stringify(mamCook.myHistory)});
				hiddenSer = $('<input>').prop({'type':'hidden','name':'search','value':searchValue});
				hiddenSrt = $('<input>').prop({'type':'hidden','name':'sort','value':sort} );    
				$('<form></form>').prop({'method':'POST','action':'search'}).append(hiddenSec, hiddenSrt, hiddenHis, hiddenPag).appendTo('body').submit();
			}
			...
			function sortRegion(region) {
				...
				hiddenSec	= $('<input>').prop({'type':'hidden','name':'search','value':'${categoryPageVO.getSearch()}'});
				$('<form></form>').attr({'method':'post','action':'regionSearch'}).append(hiddenHis, hiddenSort, hiddenPgn, hiddenRgo, hiddenSec).appendTo('body').submit();
			}
		</script>
	</body>
</html>